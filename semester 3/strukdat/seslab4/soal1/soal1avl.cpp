...
long int height(struct node *n) {  //THIS RETURNS THE HEIGHT OF ANY NODE
  if(n == NULL) 
  {
    return 0;
  }
...
long int getBalance(struct node *n) {   //THIS IS TO OBTAIN THE BALANCE BETWEEN THE NODES
  if(n == NULL) {
    return 0;
  }
...
long int findkthsmallest(struct node *root,long long int k) {
  if(root == NULL) 
  {
    return -1;
  }
...
long int findRank(struct node *root, long long int key) {
   if(root->key == key) 
   {
...
long int smallest(struct node *root) {
